...
Original file is located at
    https://colab.research.google.com/drive/1McGS6gY1Y64v0lgQrXafmziu_lnRem_y
"""
...
try:
    ...
    print(f"   Model is on device(s): {llm_model.hf_device_map}")
...
dataset_filename = "sleep_coach_finetuning_data.jsonl"
...
if not ready_to_train:
    ...
    output_dir = "./sleep-coach-lora-adapter-mistral7b"
    num_train_epochs = 3
...
final_adapter_path = "./sleep-coach-lora-adapter-mistral7b/final_adapter"
    ...
    while True:
        ...
        if user_input_text.lower().strip() == 'upload':
            ...
            if transcribed_text:
                ...
                processed_input_for_llm = transcribed_text
            else:
        ...
        if processed_input_for_llm and processed_input_for_llm.strip():
            ...
            if assistant_response_text:
              ...
              print(f"   TTS Input (Original): '{assistant_response_text}'")
              print(f"   TTS Input (Processed with Regex): '{tts_processed_text}'")
...
def napcoach_interface_handler(user_audio_filepath, user_text_query, chat_history_for_display):
    ...
    if user_audio_filepath and os.path.exists(user_audio_filepath):
        ...
        final_user_query_to_llm = transcribe_audio(user_audio_filepath)
        ...
        return chat_history_for_display, None, None, None, ""

    chat_history_for_display.append([final_user_query_to_llm, None])
    yield chat_history_for_display, None, None, None, ""
    ...
    if final_user_query_to_llm and final_user_query_to_llm != "(Audio transcription failed or was empty)":
        ...
        chat_history_for_display[-1][1] = llm_response_text
    else:
        llm_response_text = "I couldn't understand the audio. Could you please type your query?"
        chat_history_for_display[-1][1] = llm_response_text

    yield chat_history_for_display, llm_response_text, None, None, ""
    ...
    if llm_response_text:
        ...
        tts_output_audio_filepath = synthesize_for_gradio_and_autoplay(llm_response_text)
        prpgress_bar.update(1.0, desc="Audio ready!")
...
with gr.Blocks(theme=gr.themes.Glass(), title="NapCoach AI") as demo:
    gr.Markdown(
        """
        # 🎙️ NapCoach AI 💤
        Your personal sleep coaching assistant.
        Ask questions via text or by recording/uploading your voice.
        The coach will reply with both text and voice!
        """
    )
    ...
    with gr.Row():
        audio_in_ui = gr.Audio(
            ...
            type="filepath",
            label="Speak or Upload Audio Query",
        )
        text_in_ui = gr.Textbox(
            label="Or Type Your Query Here",
            placeholder="e.g., How can I sleep better tonight?",
            lines=3,
            show_label=True
        )

    submit_btn_ui = gr.Button("Send to NapCoach", variant="primary")

    gr.Markdown("### Listen to Coach's Reply:")
    tts_audio_out_ui = gr.Audio(
        label="NapCoach Voice",
        type="filepath",
        autoplay=True,
        interactive=False
    )
    ...
    clear_btn_ui = gr.ClearButton(
        components=[audio_in_ui, text_in_ui, chatbot_ui, llm_text_for_display, tts_audio_out_ui],
        value="Clear All"
    )
    ...
    def clear_internal_llm_history_action_ui_v2():
        ...
        return "LLM's internal memory cleared. Start a new conversation with NapCoach."

    clear_llm_mem_btn_ui = gr.Button("Clear Coach's Internal Memory")
    status_text_ui = gr.Textbox(label="Status", interactive=False, visible=True, lines=1, max_lines=1)
    clear_llm_mem_btn_ui.click(fn=clear_internal_llm_history_action_ui_v2, inputs=None, outputs=[status_text_ui])

    gradio_inputs = [audio_in_ui, text_in_ui, chatbot_ui]
    gradio_outputs = [chatbot_ui, llm_text_for_display, tts_audio_out_ui,
                      audio_in_ui, text_in_ui]
    text_in_ui.submit(
